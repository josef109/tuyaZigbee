...
_CODE_ZCL_ static status_t zcl_windowCovering_clientCmdHandler(zclIncoming_t *pInMsg)
{
	...
	zcl_winCovering_cmdPayload_t cmdPayload;
	memset((u8 *)&cmdPayload, 0, sizeof(zcl_winCovering_cmdPayload_t));
	...
	switch(pInMsg->hdr.cmd)
	{
		...
		case ZCL_CMD_GO_TO_LIFT_PERCENTAGE:
			cmdPayload.goToLiftPer.perLiftValue = *pData;
			break;
		case ZCL_CMD_GO_TO_TILT_PERCENTAGE:
			cmdPayload.goToTiltPer.perTiltValue = *pData;
			break;
		case ZCL_CMD_GO_TO_LIFT_VALUE:
			cmdPayload.goToLiftValue.liftValue = BUILD_U16(pData[0], pData[1]);
			break;
		case ZCL_CMD_GO_TO_TILT_VALUE:			
			cmdPayload.goToTiltValue.tiltValue = BUILD_U16(pData[0], pData[1]);
			break;
		default:
			status = ZCL_STA_UNSUP_CLUSTER_COMMAND;
			break;
